...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 01-01-01 | 01 | 1 | DATA-01 | T-01-02 | Style preview uses `previewMediaId`, not raw storage keys | schema contract | `rg "model GlobalStyle|previewMediaId|styleAssetId" prisma/schema.prisma prisma/schema.sqlit.prisma` | ❌ W0 | ⬜ pending |
| 01-01-02 | 01 | 1 | DATA-02 | — | Existing `artStyle` and `artStylePrompt` remain present | schema contract | `rg "styleAssetId|artStylePrompt|artStyle" prisma/schema.prisma prisma/schema.sqlit.prisma` | ❌ W0 | ⬜ pending |
| 01-02-01 | 02 | 1 | DATA-03, MIG-02 | — | Built-in styles expose stable legacy keys and deterministic fallback | unit | `npx vitest run tests/unit/style/legacy-system-styles.test.ts` | ❌ W0 | ⬜ pending |
| 01-02-02 | 02 | 1 | DATA-04, MIG-01, MIG-03, MIG-04 | T-01-01 | Missing/inaccessible style assets fall back without leaking ownership | unit | `npx vitest run tests/unit/style/resolve-style-context.test.ts` | ❌ W0 | ⬜ pending |
| 01-02-03 | 02 | 1 | DATA-05 | T-01-04 | Task snapshot is serializable and stable after creation | unit | `npx vitest run tests/unit/style/snapshot.test.ts` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/unit/style/legacy-system-styles.test.ts` — stubs and assertions for DATA-03 and MIG-02
- [ ] `tests/unit/style/resolve-style-context.test.ts` — stubs and assertions for DATA-04, MIG-01, MIG-03, and MIG-04
- [ ] `tests/unit/style/snapshot.test.ts` — stubs and assertions for DATA-05
- [ ] Schema contract check for DATA-01 and DATA-02 via `rg` or a lightweight test that reads both Prisma schema files
