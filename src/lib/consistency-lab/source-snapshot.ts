...
import {
  ...
  type ConsistencyLabModelConfigSnapshot,
  type ConsistencyLabSourceSnapshot,
  type ConsistencyLabSourceVideoBlock,
} from './types'
...
function buildVideoBlockId(editScriptId: string, blockIndex: number): string {
  return `${editScriptId}:videoBlock:${blockIndex + 1}`
}

function requireModelConfig(config: Awaited<ReturnType<typeof getProjectModelConfig>>): ConsistencyLabModelConfigSnapshot {
  if (!config.analysisModel || !config.storyboardModel) {
    throw new ApiError('INVALID_PARAMS', {
      code: 'CONSISTENCY_LAB_MODEL_NOT_CONFIGURED',
      message: 'Analysis model and storyboard image model are required before creating consistency lab runs',
    })
  }
  return {
    ...
    singleShotVideoModel: config.singleShotVideoModel ?? null,
    sequenceVideoModel: config.sequenceVideoModel ?? null,
  }
}
...
async function resolveAssetPreview(requirement: EditAssetRequirement): Promise<string | null> {
  ...
  if (requirement.kind === 'character') {
    ...
    const imageUrls = decodeImageUrlsFromDb(appearance.imageUrls, 'consistencyLab.character.imageUrls')
...
async function buildAssetSnapshots(requirements: readonly EditAssetRequirement[]): Promise<ConsistencyLabAssetSnapshot[]> {
  ...
  return await Promise.all(requirements.map(async (requirement) => {
    if (!requirement.id || !requirement.targetId) {
      throw new Error(`CONSISTENCY_LAB_ASSET_INVALID:${requirement.name}`)
    }
    return {
...
export async function buildConsistencyLabSource(input: {
  readonly projectId: string
  readonly episodeId: string
  readonly editScriptId: string
  readonly userId: string
}): Promise<{
  readonly sourceSnapshot: ConsistencyLabSourceSnapshot
  readonly modelConfigSnapshot: ConsistencyLabModelConfigSnapshot
}> {
  ...
  if (editScript.status !== 'ready') {
    throw new ApiError('CONFLICT', {
      code: 'CONSISTENCY_LAB_EDIT_SCRIPT_NOT_READY',
      message: 'Consistency lab requires a ready edit script',
    })
  }
  if (!editScript.id) throw new Error('CONSISTENCY_LAB_EDIT_SCRIPT_ID_REQUIRED')
  const modelConfigSnapshot = requireModelConfig(config)
  const assets = await buildAssetSnapshots(editScript.requirements)
  const videoBlocks: ConsistencyLabSourceVideoBlock[] = editScript.videoBlocks.map((block, blockIndex) => ({
    ...block,
    blockIndex,
    sourceVideoBlockId: buildVideoBlockId(editScript.id ?? input.editScriptId, blockIndex),
  }))
  return {
    modelConfigSnapshot,
    sourceSnapshot: {
