...
import { manualCreditBalance } from '@/lib/admin/billing'
...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  let result: Awaited<ReturnType<typeof manualCreditBalance>>
  let userId: string
  try {
    userId = requiredString(context.body.userId, 'userId')
    result = await manualCreditBalance({
      userId,
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Invalid manual credit' }, { status: 400 })
  }
