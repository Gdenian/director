...
import { refundAdminOrder } from '@/lib/admin/billing'
...
export const POST = apiHandler(async (
  ...
  let result: Awaited<ReturnType<typeof refundAdminOrder>>
  try {
    result = await refundAdminOrder({
      orderId,
      reason: context.reason,
      operatorId: context.actorId,
    })
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Invalid order refund' }, { status: 400 })
  }
