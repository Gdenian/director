...
type MethodRecord = Record<string, unknown>
...
function autoBind(instance: object) {
  const proto = Object.getPrototypeOf(instance) as MethodRecord
  Object.getOwnPropertyNames(proto).forEach((key) => {
    const value = proto[key]
    if (key !== 'constructor' && typeof value === 'function') {
      ;(instance as MethodRecord)[key] = value.bind(instance)
    }
  })
}
...
class GalleryApp {
  ...
  onWheel(e: WheelEvent) {
    ...
    this.scroll.target += ((delta ?? 0) > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2
...
export default function CircularGallery({
  ...
  return (
    <div
      className={styles['circular-gallery']}
      ref={containerRef}
      onPointerDown={(event) => {
