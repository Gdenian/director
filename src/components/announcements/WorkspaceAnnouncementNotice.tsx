...
export function WorkspaceAnnouncementNotice() {
  const locale = useLocale()
  const [items, setItems] = useState<AnnouncementItem[]>([])

  useEffect(() => {
    let mounted = true
    fetch(`/api/announcements?surface=workspace_notice&locale=${encodeURIComponent(locale)}`, { headers: { Accept: 'application/json' } })
